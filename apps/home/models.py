...
class Avancement(models.Model):
    employee = models.ForeignKey(Employe, on_delete=models.CASCADE, null=True, blank=True)
    teacher = models.ForeignKey(Enseignant, on_delete=models.CASCADE, null=True, blank=True)
    grade = models.ForeignKey(Grade, on_delete=models.CASCADE)
    contrat = models.ForeignKey(Contrat, on_delete=models.CASCADE)
    date_effective = models.DateField()

    def avance_employee_grade(self):
        if self.employee:
            date_today = timezone.now().date()
            contrat_duration = date_today - self.employee.date_PF

            if contrat_duration >= timedelta(days=365):
                new_grade = self.employee.grade
                echellon_actuel = int(new_grade.num_echellon)
                classe_actuelle = int(new_grade.num_classe)
                if echellon_actuel < 3:
                    new_grade.num_echellon = str(echellon_actuel + 1)
                else:
                    new_grade.num_echellon = '1'
                    if classe_actuelle < 3:
                        new_grade.num_classe = str(classe_actuelle + 1)

                new_grade.save()

    def avance_teacher_grade(self):
        if self.teacher:
            date_actuelle = timezone.now().date()
            date_debut_service = self.teacher.date_PF
            deux_ans_plus_tard = date_debut_service + timedelta(days=730)

            if date_actuelle >= deux_ans_plus_tard:
                new_grade = self.teacher.grade
                echellon_actuel = int(new_grade.num_echellon)
                classe_actuelle = int(new_grade.num_classe)
                ...
                if echellon_actuel < 3:
                    ...
                    new_grade.num_echellon = '1'
                    if classe_actuelle > 1:
                        new_grade.num_classe = str(classe_actuelle - 1)

                new_grade.save()

    def get_new_avancement_date(self):
        return self.date_effective + timedelta(days=365)

    def get_new_grade(self):
        if self.employee:
            new_grade = self.employee.grade
            num_classe = str(new_grade.num_classe) + 'C' if new_grade.num_classe >= 10 else '0' + str(new_grade.num_classe) + 'C'
            num_echellon = str(new_grade.num_echellon) + 'E'
            return f'{num_classe}{num_echellon}'
