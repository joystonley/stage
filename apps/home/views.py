...
from .forms import AvancementForm
...
def index(request):
    ...
    context['nombre_conges'] = nombre_conges
     # Ajout du nombre d'avancements au contexte
...
# Vue pour modifier un congé existant (Update)
...
def modifier_conge(request, id_conge):
    ...
    employes = Employe.objects.all()  # Assurez-vous de récupérer la liste des employés
    enseignants = Enseignant.objects.all()  # Assurez-vous de récupérer la liste des enseignants
    typeconges = TypeConge.objects.all()  # Assurez-vous de récupérer la liste des types de congés
...
def liste_avancements(request):
    avancements = Avancement.objects.all()
    return render(request, 'avancements/liste_avancements.html', {'avancements': avancements})




@login_required(login_url="/login/")
def ajouter_avancement(request):
    if request.method == 'POST':
        form = AvancementForm(request.POST)
        if form.is_valid():
            avancement_instance = form.save()  # Sauvegarde l'instance dans la base de données
            messages.success(request, 'Avancement ajoutée avec succès.')
            return redirect('liste_avancements')
        else:
            messages.error(request, 'Il y a des erreurs dans le formulaire.')
    else:
        form = AvancementForm()
    ...
    return render(request, 'avancements/ajouter_avancement.html', {
        'form': form,
        'employes': employes,
        'enseignants': enseignants,  # Correction ici
        'grades': grades,  # Correction ici
        'contrats': contrats,  # Correction ici
    })

@login_required(login_url="/login/")
def modifier_avancement(request, pk):
    avancement = get_object_or_404(Avancement, pk=pk)

    if request.method == 'POST':
        form = AvancementForm(request.POST, instance=avancement)
        if form.is_valid():
            avancement = form.save()
            messages.success(request, 'Avancement modifiée avec succès.')
            return redirect('liste_avancements')
        else:
            messages.error(request, 'Il y a des erreurs dans le formulaire.')
    else:
    ...
    return render(request, 'avancements/modifier_avancement.html', {
        'form': form,
        'employe': employes,
...
def supprimer_avancement(request, pk):
    avancement = get_object_or_404(Avancement, pk=pk)
    avancement.delete()
    return redirect('liste_avancements')
